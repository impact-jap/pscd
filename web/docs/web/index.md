...
Correct ZIP layout with an existing `SYSTEM.CNF`:
...
Correct minimal homebrew-oriented ZIP layout:
...
If `SYSTEM.CNF` exists at the uploaded project root, the Web/WASM builder uses it as-is. If it is missing and a root boot ELF can be detected, the builder generates `SYSTEM.CNF` inside the virtual build input.
...
The Web/WASM preview uses the same packed legacy layout by default. It targets the boot executable at LBA `12231` and packs real project files before that anchor when they fit.

The Web configuration lives in `web/src/js/config.js`:
